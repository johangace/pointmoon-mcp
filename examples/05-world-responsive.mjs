...
assert.ok(payload.notices && !Array.isArray(payload.notices), 'Expected the documented object-shaped notices')
assert.ok(Array.isArray(payload.notices.sources), 'Expected source notices')
assert.deepEqual(context.notices, payload.notices, 'Projection must preserve attribution notices')
